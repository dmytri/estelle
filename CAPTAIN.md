...
- **Runnable package, iteration 5.** The three seam scenarios landed as `bd7344c`: the `estelle` bin plus `build` script, and assetless-directory boot resolving package-shipped assets (Bonny on `opencode-go/deepseek-v4-flash`, extension loaded). Suite 46/46 `@logic`, tsc and biome green.
- **Outbound artifact check found a packaging gap, blocking publish.** `npm pack --dry-run` showed the tarball excludes `dist/` (gitignored, no `.npmignore`), so the published `bin/estelle.js` `require("../dist/index.js")` cannot boot. It also ships `CAPTAIN.md` and dev-only files (`features/**`, `src/`, `RIGGING.md`, tooling configs, `watchbill.json`) to the public registry, a privacy leak. The seam scenarios verified the bin and the boot path through `src`, not the packed artifact, so this slipped.
- **Decision: spec the packed artifact, then publish.** New scenario `The published package ships its runtime and withholds Captain notes` in `features/runnable-package.feature` asserts the packed tarball includes `dist/index.js`, `bin/estelle.js`, and a shipped asset, and withholds `CAPTAIN.md` and `src/index.ts`. Focused by `watchbill.json` watch1 for the next `/qm` cycle. The packaging fix is Crew production: add `main: "dist/index.js"` and `files: ["dist","bin","assets"]` to `package.json`; the allowlist is the clean seam, auto-excluding everything else. Publish stays held until this lands, the manual outbound boot check passes, and `version` is bumped off `0.0.0`. npm auth present as `dk`.
