...
- Open: Bonny autonomously EMBARKING on confirmed intent (`@eval` "Bonny embarks... instead of sending the operator to a role command") is RED on `deepseek-v4-flash` even with a maximally forceful card; it competes with the upstream captain skill's "switch to execution". Small-model instruction-following limit, not a wiring bug. Operator decision pending: accept best-effort (works on a capable model) and rely on manual alongside dispatch, or point `HARNESS_EVAL_MODEL` at a stronger model to require it.
- Deferred hygiene: ~4 stale seat-switch `@planks` a Boatswain was mid-removing when a session-limit API error killed the cycle; and pre-existing stale planks. Not committed. Crew-flow B+C shipped in `0.1.13` (registry boot-verified); A embark and the plank hygiene remain.
