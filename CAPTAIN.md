...
- Slice 2 handed off: the crew's heartbeat. `@logic` pins the heartbeat surface naming the seat and its at-rest state; `@eval` pins that a live crew turn drives the heartbeat off the real event stream. Crew adds a heartbeat state fed by subscribing to the crew session's event stream, exposed on `crewSession()`. QM writes the `@eval` harness support that loads the required credentials from `.env`; no skip gate. The literal panel rendering and `Alt+/` focus toggle are terminal keybindings, not cucumber-testable; they ride the published-artifact boot check, not this spec.
