...
- Slice 5 handed off: report-back, the last live-crew slice. When the crew's run ends Estelle summarizes the result back into Bonny's session so she can speak to it, while the firewall holds (distilled summary, never the crew's raw context). `@logic` pins the report seam plus the firewall (Bonny's history records a report but excludes the crew's raw message); `@eval` pins a real model summary. Crew adds a report seam that injects a distilled summary into the started session, voiced by a live Bonny call when a captain model is available. Completing slice 5 finishes the live-crew arc (arc 1); then outbound decision, then arcs 2 and 3.
