...
- `@eval` tier established 2026-07-04, modelled on `~/jolly`'s live-agent eval. Opt-in, excluded from default/broad, skip-not-fail when `HARNESS_OPENROUTER_API_KEY` absent, never a gate. Knobs in gitignored `.env` (`HARNESS_OPENROUTER_API_KEY` reused from jolly, `HARNESS_EVAL_MODEL=deepseek/deepseek-v4-flash`); template in `.env.example`; `eval` command + tier in `RIGGING.md`. The live-crew heartbeat/narration slices verify here; `@logic` keeps the deterministic plumbing. `.env` gitignored per operator instruction.
- Slice 2 handed off: the crew's heartbeat. `@logic` pins the heartbeat surface naming the seat and its at-rest state; `@eval` pins that a live crew turn drives the heartbeat off the real event stream. Crew adds a heartbeat state fed by subscribing to the crew session's event stream, exposed on `crewSession()`. QM writes the `@eval` harness support (model on jolly `features/support/eval.ts`) and the skip-not-fail gate. The literal panel rendering and `Alt+/` focus toggle are terminal keybindings, not cucumber-testable; they ride the published-artifact boot check, not this spec.
