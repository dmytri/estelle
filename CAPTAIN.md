...
- Specced in `features/greeting.feature`, `features/seat-model-defaults.feature`, `features/seat-model-selection.feature`; `features/seat-model-fallback.feature` survives unchanged (fallback target is now an available model, steps already say so). `runnable-package` scenario "booted Captain runs on the shipped default model" struck with the shipped default. First cycle landed green (`95845d0`).
- **Hold before 0.1.4: the interactive path ignores recorded models.** Boatswain evidence: `run()` builds fresh state with empty `seatModels` and never reads `estelle.json`; recorded models bind only the `launch()` seam. Same `launch()`/`run()` divergence class as the TUI and built-ins bugs. Specced `A recorded seat model binds the started session` in `seat-model-selection.feature`; watchbill focuses it. A stale deepseek plank on `beginTurn` (src/index.ts:637) rides along for correction; the cycle's plank audit clears it. Ship 0.1.4 when green.
