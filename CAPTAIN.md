...
- **Publish held on three gates only.** Spec side is done. Before `npm publish --access public`: bump `version` off `0.0.0`, run `pnpm build`, then the manual outbound boot check per RIGGING Outbound (published package launches pi as Bonny with the extension loaded). npm auth present as `dk`. Local commits are ahead of origin; push is the other pending outbound.
