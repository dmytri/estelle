...
- Release: **`@dk/estelle@0.1.20` shipped and boot-verified** (commit `ab39936`, tag `v0.1.20`). Registry install pulled shim `0.1.1` correctly; strace-proved the real bin reads its bundled `agent-prompts.json`, not a cwd decoy, and launched pi with Bonny seated. **Shim `pi-open-plugin-shim` left at `0.1.1`, not republished** — its only unreleased change since `0.1.1` was `@planks` docblock comments (commit `0e6cab9`), zero runtime change, so the compiled artifact is byte-identical; the flagship's `workspace:*` resolves to the published `0.1.1`. Next release that touches shim runtime must bump it.
