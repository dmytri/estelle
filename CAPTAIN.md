...
- **Release:** `@dk/estelle@0.1.22` shipped and boot-verified (`8f21bea`, tag `v0.1.22`, pushed with `main`). Registry install in a decoy-`assets/` project: `workspace:*` rewrote to `pi-open-plugin-shim@0.1.1`; published dist carries the captain-branch `embarkSteer` append and the bundled catalog carries the steer copy; real bin booted pi v0.80.6 as Bonny with the Shipshape skills loaded in a fresh namespaced agent dir (`PI_CODING_AGENT_DIR`); passthrough seam live from the registry (`--help` forwarded, `-p` completed a real model turn); strace confirmed the bin reads the BUNDLED `agent-prompts.json` in the decoy condition. The embark behaviour itself was proven pre-release by the `@eval` focused reruns at `2e9f108` (same src as the release commit). Shim untouched, stays `0.1.1`; the next release touching shim runtime must bump it. Boot-verify craft for next time: argv-bearing invocations are pure passthrough by design (no Bonny, no seat prompt), and the bare-launch TUI writes no session file until its first turn, so a prompt-content assertion at boot needs either one real turn or the dist+catalog+strace chain.
