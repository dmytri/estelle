...
**Content-catalog completion voyage (in flight, this Captain turn):** Shipwright found more agent-facing copy still embedded in `src/index.ts`, against `content-catalog.feature`'s own Rule. Operator chose promote-via-catalog. Catalogued the five plain-string sites into `assets/agent-prompts.json`: `crewLoopPrompts.{quartermaster,crew,boatswain,crewReady}` (drive-loop seat turns `:1379/:1386/:1398` plus crew-ready `:1511`) and `embarkComplete` (`:420`). The existing "Catalogued agent-prompt copy is not duplicated" `@property` check (methodology-conformance.feature, substring scan over the flattened catalog) now reds against the inline literals; that is the M4 driver. QM discovers the red, Crew relocates the five seams to read from the catalog. Removed the two `@captain` skeletons: their intent is served by the catalog plus the existing check (same as `openingTurn`/`crewRunSummary`/`crewRunNarration`, which carry no per-site scenario), so per-site equality scenarios would re-add the over-specification removed in 0.11.1. **Next: clear context, then QM.**
...
- **Release:** `@dk/estelle@0.1.20` shipped and boot-verified (`ab39936`, tag `v0.1.20`); shim `pi-open-plugin-shim` held at `0.1.1` (only unreleased change is `@planks` docblocks, byte-identical compiled artifact). The next release that touches shim runtime must bump it.
- **Economy debt → QM when touching support:** `runnable-package`/`pi-command-passthrough` rebuild `pnpm build` per scenario (~60-70s lever, no build-once guard); `internal-api-shape` 13.9s scantling attestation every inner loop (candidate tier-retag, Captain call); `pi-command-passthrough` real npm install ~37s (spec-form: does argv-forwarding need a real install or a cheaper pi subcommand).
