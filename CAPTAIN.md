...
- Slice 3 handed off: the QM to Crew handoff, Estelle-driven. `@logic` pins that handing off opens a fresh Crew-seated session isolated from the Quartermaster's context and carrying Crew custody (`src/**` only); `@eval` pins a real Quartermaster turn handing off to a fresh Crew session. Crew adds the handoff seam (open a fresh crew sub-session seated as a roster Crew hand, custody following the seat); the full loop-until-green is a later slice.
