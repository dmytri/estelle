...
## Open finding 2026-07-08: embark is vacuous in the live run (fix = voyage (a))

Operator field-tested `0.1.15` embark on a real task; the crew never worked. Confirmed in `src/index.ts`: the registered `embark` tool (`:364`) and the `/embark` + `/qm` commands (`:337`, `:344`) call only `state.openCrewSession()` (`:1132`), which builds an idle Misson runtime and returns. Nothing drives the loop, verification, or report. The real loop `runCrewLoopToCompletion` (`:1408`), the `captainTools` embark that narrates and reports (`:1344`), and narration/report are reachable only from `features/steps/live-crew.steps.ts` (`:805`, `:861`). So the `@eval` "a live Bonny embark runs the crew loop to green" scenario is a FALSE GREEN: it calls test-facing methods, not the registered tool the live model uses. Fifth instance of the recurring test-path-diverges-from-live-path pattern (`0.1.12` was the same bug). Shipped in `0.1.15`.

Deeper: even `runCrewLoopToCompletion` is an `@eval` demo, not a real crew loop. It drives three seats to voice one canned line each ("Do not read files. Do not call any tools.") and writes the fixture `estelle-live-target.md`. It runs no verification and makes no real project change.

Fix = voyage (a) (operator, 2026-07-08). Build the real crew loop: embark drives the crew seats, each running their real Shipshape role, through a real QM->Crew->Boatswain loop that runs the project's real verification and makes real changes until green, narrating and reporting into the operator's session. The proving scenario MUST drive the registered embark tool (the live path, like the custody tests drive the real hook via `emitToolCall`) and assert an outcome only the real path can produce (the failing target actually passes real verification), so it cannot go vacuous again. The (b) honest-message stopgap was dropped: Estelle is not public, only the operator tests it, so (a) makes the claim true and fully supersedes any interim.

Boot-verify gap to close: `0.1.15` boot-verify confirmed launch-as-Bonny but never exercised embark. A release check should drive embark on a real target.

Confirmed root cause (operator field-tested 0.1.15, 2026-07-08): `state.openCrewSession` IS assigned in the live `launch()` (`src/index.ts:1132`), so `/embark`, `/qm`, and the embark tool all call it, but it only builds an in-process, invisible, idle Misson runtime (`:1144`) and returns. Nothing drives that session and nothing surfaces into the operator's TUI. So the ENTIRE crew surface is dead in the live run, manual and autonomous alike: to the operator, every crew command does nothing. The loop, narration, and report that would do work and show it (`runCrewLoopToCompletion`, `captainTools`, `narrateCrewRun`, `reportCrewRun`) exist only as test-handle methods the live path never calls. There is no usable crew release; parking gains nothing.

Spec-first attempt reverted (was at the failed QM pass): reworking the `@eval` scenarios before a real loop existed orphaned 8 planks on the demo seams and the `@logic` tier proved unable to force real work (Crew satisfied a new `@logic` pin via the test-method path). Lesson: (a) has no `@logic` shortcut, and the build must be additive: build the real loop plus its `@eval` proof first, retire the demo seams after. Tree reverted to `0d6cfc4`.

(a) real scope: the live commands and the embark tool must drive a real crew loop that runs the project's verification and drives the crew seats to make real changes to green, and surface narration and the run report into the operator's own session. Proven by an `@eval` scenario that drives the registered tool and asserts the project's real verification goes red to green. This is the core "crew runs the real Shipshape roles via the shim, doing real work" capability, not a wiring patch.
