...
## Decision 2026-07-09: fail-fast/perturb key reconciled deliberately, migrating code

Same rename this note flagged before (`## Perturbation` key `fail-fast` -> `perturb`, matching the generic Shipwright read contract) recurred in a later Shipwright harbour pass, unchecked this time since Shipwright never reads this file. Rather than revert a third time, decided with the operator to reconcile deliberately: keep `RIGGING.md`'s `perturb` key, retire `fail-fast` from vocabulary project-wide. Renamed `features/perturb-command.feature`'s "fail-fast statement" step text and prose to "perturbation statement" to match. `riggingFailFast()` in `src/index.ts:237` still reads the stale `fail-fast:` key and now fails `features/perturb-command.feature:14` for real; this is the intended failing verification target, not a regression to hide. QM discovers it fresh and dispatches Crew to rename the seam's lookup, likely `riggingFailFast` itself, to read `perturb`. No perturbation statement added at the seam since it is already red from this change, not merely stale-but-passing.
