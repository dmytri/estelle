...
Given("the survivor roster in {string}", function (this: EstelleWorld, path: string) {
	const roster = JSON.parse(readFileSync(path, "utf8")) as { survivors: string[] };
	this.roster = roster.survivors;
	assert.ok(Array.isArray(this.roster) && this.roster.length > 0, "survivor roster is empty");
});
...
Then("the hand name appears in {string}", function (this: EstelleWorld, path: string) {
	const roster = JSON.parse(readFileSync(path, "utf8")) as { survivors: string[] };
...
Then("the name is present before the hand first provider request", function (this: EstelleWorld) {
	assert.ok(this.seat, "no Crew hand was seated");
	assert.ok(this.seat.name.length > 0, "the seated Crew hand has no name");
	assert.equal(
